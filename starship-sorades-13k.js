...
submitBtn.addEventListener('click', function() { submitGist();} , false);
...
function submitGist() {
	...
	_data = {
  	...
  	fetch('//skogaming.com/api/submitScores', {
    method: 'post',
...
function submitShots() {
	...
	shotsFired = 0;
  	fetch('//skogaming.com/api/submitShots', {
    method: 'post',
    body: JSON.stringify(_shots),
    contentType: 'application/json'
  	}).then(function(response) {
    	return response.json();
  	}).then(function(data) {
    	console.log('Submitted Shots!');
  	});
     
    loop_handle=setTimeout(submitShots,5000);
